...
In the post I wrote last year about the lessons learned from building a table component I mentioned just a a :astro-ref[few lines]{path="blog/2024/2024-10-21-table-component-ii" fragment="row-selection"} about the selection, but I think it is worth to write a separate post about it as it is an interesting topic.


## What is chunked data in this context?

In this context "chunked data" refers you will not have all the data at the same time in the client and you will have only access to a subset of the data at a time, and to some metadata, for example, the total number of rows.

This is a very common scenario when when you are dealing with large datasets, or when you want to give the user the fastest possible response time.

> If you check the previous post I mentioned the internal pagination, that means the data is chucked into page, but it a UI chunking, not a data chunking as you have all the data available in the client and you chuck it for render porpouses, so it is not the same case.
...
if you need to execute an action on the selected rows in the backend you just send the array of selected rows (in most cases its better to send the ids of the selected rows). And the backend can just execute the action.
...
- You need to keep track of the selected rows in the client, and agains will need more memory and network usage as bigget is the dataset.
...
Imagine you have a table with 1B rows, this probably  will be unmanageable in the client (I pretty sure it will be), so the reasonable thing to do is to chunk the data, for example, in chucks (pages) of 1000 rows. Doing this we have a maximun use case, no mattrer how many rows you have, you will only have 1000 rows in the client at a time, so the memory usage, CPU usage, and network usage for each load have a upper limit.
...
But in terms of User Experience this forces the user to visit all the pages to select all the rows, which is one of the worse UX i can imagine.

Tipically we will have a select all checkbox or button, that allows the user to select all INDICATE WE WANTS to select all the rows, event the one are not loaded.
...
#### Select all rows
As we mentioned, the user can have a button or a checkbox (lets use this in the examples) to select/deselect all rows, in this case we need to store the status of this checkbox (and send it to the backend later)
...
With this simple struct we can represent the user desires, and we can send it to the backend, so the backend can recreate the real list of selected items,
...
One we have the `selectedStatus` object and the access to query the items (for example in the backend) we can recreate the real list of selected items. The logic is simple, should start from the wide scope to the narrow scope, apply the exceptions (that are the status of the childs scope) and **understand want the undefined means in the context of the selection, that is to follow the parent desired state**.
...
Starting for the wider scope, the `all` property, it is true, so we will select all the items so any children (group or row) that is not in the `selectedStatus` will be selected. In this case we should the items add the group 1 (because it is explicity selected) and 3 (because it is not explicity deselected).

Next iteration is focused in the items:
- the rows 1 and 2 (group 1) are explicity deselected, so we will remove them to the selected items in the group 1.
- the row 3 (group 2) is selected, so we will add it to the selected items even if the group 2 is explicity now selected.
