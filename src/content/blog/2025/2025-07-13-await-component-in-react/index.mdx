...
The simplies solution is to use useState and useEffect in your component to manage the loading, and error state, and wait for the promise to resolve:
...
As this is a very common use case, there are many libraries that provide a solution for this problem, like [react-async](https://github.com/async-library/react-async), which provides a hook and a component to manage the loading states.

Or TanStack Router's [Await component](https://tanstack.com/router/v1/docs/framework/react/api/router/awaitComponent), a component part of the TanStack Router library, that provides a component to wait for a promise to resolve and render the UI accordingly.
...
In my use case, TanStack Await component seemed to be the solution, even if I don't use TanStack Router, but I found our use case was a little bit different.
...
The use cases for await component is a good example of this.

We have a component that renders a table with groups, and the groups renders in the UI the group name and the number of items in the group. Depending on the data source the number of items in the group can be a number or a promise that resolves to a number. 

One way to handle this is to render the Await component if the value is a promise, and render the value directly if it is not a promise, but this forces us to add the logic in every place we want to use the value. So whay not thinking in a "primitive" value like a promise that is resolved intantly.
...
That it something the TanStack Await component does not, so I decided to implement our own Await component, that can handle both cases, and also provide a loading state and an error state.

The component is very simple, it accepts:
- a `resolve` prop that can be a promise or a value that is the value we want to await for, 
...
```
And it works in the same way when the resolve props is a real value, like a number or a string:
