...
A diferencia de los chatbots estándar o agentes como OpenClaw que manejan una solicitud a la vez, Spacebot está diseñado para:

- **Manejar concurrencia**: Puede participar en docenas de conversaciones simultáneamente a través de diferentes canales sin "bloquearse" ni ralentizarse, utilizando el concepto de ramas (branches) para gestionar diferentes conversaciones o diferentes solicitudes dentro de la misma conversación.

- **Retener memoria**: Mantiene un grafo de memoria, puede aprender de las interacciones, o puedes ingerir información en él y utilizar esa información en futuras interacciones.

- **Ejecución de tareas**: Puede ejecutar tareas, como navegar por la web, ejecutar código o interactuar con APIs, lo que lo convierte en una herramienta poderosa para automatizar flujos de trabajo complejos.
...
- **UI/Asistente**: Spacebot tiene una interfaz web donde puedes ver y gestionar los agentes, los workers, sus interacciones, el grafo de memoria, configurar los proveedores, los canales y más. También tiene un asistente de IA integrado que puede ayudarte a crear agentes, configurarlos y más, simplemente preguntándole.

## Agentes

Cada agente en Spacebot se puede personalizar con su propia IDENTITY, SOUL y ROLE, lo que te permite crear agentes con diferentes personalidades. Cada agente también tiene su propia memoria, espacio de trabajo y datos, aislados de los demás.

## Orquestación de agentes (jerarquía)

A diferencia de otras herramientas que pueden ejecutar agentes, Spacebot está diseñado para ejecutar múltiples agentes al mismo tiempo y definir una jerarquía entre ellos, con un grafo claro de interacciones.

![Spacebot agents hierarchy graph](./dashboard.png)

Puedes definir los enlaces entre los agentes y la dirección del flujo de información, permitiendo que los agentes deleguen tareas a otros agentes y reciban los resultados de vuelta, creando una poderosa orquestación de agentes.

## Un ejemplo

En mi caso, tengo un agente principal llamado "AmbrosIA", un agente especializado en controlar mi casa a través de Home Assistant llamado "Teixugo", un agente especializado en fitness llamado Gym AI Trainer, y un agente de broma llamado "Grumpy Sergio", que es una versión aún más gruñona de mí que uso para divertirme.
...
Cortex es un proceso LLM que se ejecuta en segundo plano y es responsable de decidir qué información en cada conversación es importante para el futuro, y de almacenar esa información en el grafo de memoria, permitiendo que los agentes tengan memoria a largo plazo de las interacciones y utilicen esa información en futuras interacciones.

![Spacebot memory graph](./memories.png)

Pero también puedes ingerir información manualmente en el grafo de memoria simplemente soltando archivos en la interfaz de usuario, y Spacebot los fragmentará (chunking), los procesará a través de herramientas de memoria LLM y producirá el grafo de memoria.

Por ejemplo, imagina que estás creando un agente de soporte para un producto. Puedes ingerir la documentación del producto en el grafo de memoria, y el agente podrá usar esa información para responder preguntas relacionadas con el producto, o incluso usar el navegador para seguir las instrucciones de la documentación para resolver problemas.
...
El add-on está disponible en mi repositorio, y puedes instalarlo en tu instancia de Home Assistant a través de HACS añadiendo mi repositorio como un repositorio personalizado y luego buscando Spacebot en la sección de add-ons:

1. Abre Home Assistant.
2. Ve a **Ajustes** → **Apps** → **App store**.
...
**Con esta configuración sencilla, puedes tener tu propio gestor de agentes de IA funcionando las 24 horas, los 7 días de la semana en tu Raspberry Pi, siempre encendido y listo para interactuar contigo a través de tus canales preferidos, y con la capacidad de crear orquestaciones complejas de agentes para automatizar tareas, controlar tu hogar inteligente o simplemente divertirte chateando con tus agentes personalizados.**


He dejado algunas capturas de pantalla del panel de control, el grafo de memoria y la jerarquía de agentes para que te hagas una idea de cómo se ve, pero te animo a que lo eches un vistazo y lo pruebes tú mismo, es una herramienta poderosa que puede abrir muchas posibilidades para los agentes de IA en tu vida diaria.
