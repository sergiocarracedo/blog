...
heroImageMeta:
  ...
  prompt: "pixel art style, retro game aesthetic. In the center of the frame, a 2 computer monitors displays the same schematic web browser interface represented by abstract boxes and lines. From each monitor a tag in the style of a Minecraft tag (see example image) hang, each tag have a differnt number . The background is a clean vertical gradient with a distinct horizon line, using a two-tone palette of cool colors (picking from cyan, purple, blue, green, or light green). The central monitor is surrounded by 20% empty negative space padding on all sides. High definition, crisp pixels, sharp edges, wide angle. --ar 16:9"
tags:
 - ci
 - release
 - vite
