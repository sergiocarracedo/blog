...
5 years ago I bought my :astro-ref[first mechanical keyboard]{path="/blog/2019/Impresiones-tras-un-mes-de-uso-de-un-teclado-mecanico"}, a cheap and simply one, the Krom Kernel, a 100% size keyboard with Outemu Red switches with spanish layout. During this time I change the space bar keycap twice because the plastic was not very good and the part that connects with the switch was not very reliable and it broke causing a bad experience pressing it. But was something I could fix, replacing all the keycaps with a better quality ones.
...
This has a negative counterpart: to write the characters in the _dead keys_ you need to use the space, for example to write `'` you must type the `'` and then the space. This is not direct as just press a key but you can get used to it.
...
Following the Félix's advice I deciced to use the _Gateron G Pro 3.0 Prelubed_ switches. Those are linear switches with a nice feeling, low sound level, and oriented to office and gaming.
